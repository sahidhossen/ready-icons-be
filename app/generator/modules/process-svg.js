...
const ProcessSvg = {
	...
	parseSvgImage: (data, filename) => {
		...
		if (!svg.hasAttribute('height')) {
			throw new Error(filename ? 'Missed height attribute in ' + filename : 'Missed height attribute');
		}
		if (!svg.hasAttribute('width')) {
			throw new Error(filename ? 'Missed width attribute in ' + filename : 'Missed width attribute');
		}
	...
	getRectDrawnAttr: (item, rectAsArgs = true) => {
		...
		if (rx < 1 && ry < 1) {
			return convertToString([['M', x, y], ['L', x + w, y], ['L', x + w, y + h], ['L', x, y + h], ['L', x, y], ['Z']]);
		} else if (rectAsArgs) {
			return convertToString([
				['M', x + rx, y],
				['H', x + w - rx],
	...
	processTree: (node, ignoredTags, ignoredAttrs, parentTransforms, path) => {
		...
		_.each(node.childNodes, function (item) {
			...
			// Parse nested tags
			if (item.nodeName === 'g') {
				let result = module.exports.processTree(item, ignoredTags, ignoredAttrs, transforms, path);
				path = result.path;
